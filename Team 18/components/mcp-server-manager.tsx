...
export const MCPServerManager = ({
  ...
  const removeServer = (id: string, e: React.MouseEvent) => {
    ...
    // Prevent deletion of fixed servers (from config)
    const server = servers.find((s) => s.id === id);
    if (server?.isFixed) {
      toast.error('Cannot delete server from configuration file');
      return;
    }
  ...
  const handleArgsChange = (value: string) => {
    try {
      ...
      console.log('handleArgsChange - input:', value, 'parsed:', argsArray);
      setNewServer({ ...newServer, args: argsArray });
    } catch (error) {
      // If parsing fails, just split by spaces
      const argsArray = value.split(' ').filter(Boolean);
      console.log('handleArgsChange - parse error, using split:', argsArray);
      setNewServer({ ...newServer, args: argsArray });
    }
  };
  ...
  const getServerDisplayUrl = (server: MCPServer): string => {
    ...
    return server.type === 'sse' ? server.url : `${server.command} ${server.args?.join(' ')}`;
  };
