...
import { ChevronDownIcon, ChevronUpIcon, LightbulbIcon, Sparkles, User } from 'lucide-react';
import { SpinnerIcon, CoffeeIcon } from './icons';
              ...
              case 'text':
                return (
                  <div key={`message-part-${i}`} className="flex flex-row gap-2 items-start w-full">
                    {message.role === 'user' && (
                      <div className="size-8 flex items-center rounded-full justify-center ring-1 shrink-0 ring-border bg-background order-2">
                        <User className="h-4 w-4 text-muted-foreground" />
                      </div>
                    )}
                    <div
                      className={cn('flex flex-col gap-3 w-full', {
                        'bg-secondary text-secondary-foreground px-4 py-3 rounded-2xl order-1':
                          message.role === 'user',
                      })}
                    >
