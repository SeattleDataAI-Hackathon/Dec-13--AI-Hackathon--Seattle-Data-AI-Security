...
export const ModelPicker = ({ selectedModel, setSelectedModel }: ModelPickerProps) => {
  ...
  const getCapabilityIcon = (capability: string) => {
    switch (capability.toLowerCase()) {
      ...
      case 'function calling':
        return <Cpu className="h-2.5 w-2.5" />;
      case 'lightweight':
      case 'edge':
  ...
  const getCapabilityColor = (capability: string) => {
    switch (capability.toLowerCase()) {
      ...
      case 'multilingual':
        return 'bg-teal-100 text-teal-800 dark:bg-teal-900/30 ocean:bg-teal-900/30 dark:text-teal-300';
      case 'multimodal':
        return 'bg-fuchsia-100 text-fuchsia-800 dark:bg-fuchsia-900/30 ocean:bg-fuchsia-900/30 dark:text-fuchsia-300';
      case 'function calling':
        return 'bg-violet-100 text-violet-800 dark:bg-violet-900/30 ocean:bg-violet-900/30 dark:text-violet-300';
      case 'lightweight':
      case 'edge':
        return 'bg-lime-100 text-lime-800 dark:bg-lime-900/30 ocean:bg-lime-900/30 dark:text-lime-300';
      default:
  ...
  return (
    <div className="absolute bottom-2 left-2 z-10">
      <Select value={validModelId} onValueChange={handleModelChange} defaultValue={validModelId}>
        ...
        <SelectContent
          align="start"
          className="bg-background/95 dark:bg-muted/95 ocean:bg-muted/95 backdrop-blur-sm border-border/80 rounded-lg overflow-hidden p-0 w-[280px] sm:w-[350px] md:w-[560px]"
        >
          <div className="grid grid-cols-1 sm:grid-cols-[120px_1fr] md:grid-cols-[250px_1fr] items-start">
